...
public class CitizenTestActivity extends Activity implements OnClickListener {	
	...
	private static String SERVICE_URI_RECORD_CREATE_WITH_ATTACHMENTS = "/v3/a311civicapp/records/";
	private static String SERVICE_URI_RECORD_SEARCH = "/v4/search/records/";

	private ProgressDialog accessTokenProgressDialog = null;
	private Button btnCivicWebLogin, btnCivicEmbeddedWebLogin, btnGetRecords, btnCreateRecord, btnCreateRecordWithAttachments, btnSearchRecord, btnCivicLogout, btnBack;
	...
	public void onWindowFocusChanged(boolean hasFocus) {		
		...
		if ( (hasFocus) && (appContext.accelaMobile4Citizen != null)
	...
	public void onClick(View view) {				
		String[] permissions4Authorization  = new String[] {"search_records", "get_records", "get_record", "create_record", "a311citizen_create_record"};
		String servicePath = null;			
		RequestParams requestParams = new RequestParams();
		
		switch (view.getId()) {					
			case R.id.btnCivicWebLogin:		
				appContext.accelaMobile4Citizen.authorize(permissions4Authorization,"ISLANDTON");
				break;	
			...
			case R.id.btnCivicGetRecords:	
				if (isSessionValid()) {
					...
					currentRequest = appContext.accelaMobile4Citizen.request(servicePath, requestParams, requestDelegate);
				}
				break;				
			case R.id.btnCivicCreateRecord:	
				if (isSessionValid()) {
					...
					currentRequest = appContext.accelaMobile4Citizen.request(servicePath, null, HTTPMethod.POST,  requestParams, requestDelegate);
				}
				break;
			case R.id.btnCreateRecordWithAttachments:
				if (isSessionValid()) {
					servicePath = SERVICE_URI_RECORD_CREATE_WITH_ATTACHMENTS;	
					// Populate JSON to be posted.		
//					requestParams = new RequestParams(getJson4CreateRecordWithAttachment());	
					requestParams.put("json-data", getJson4CreateRecordWithAttachment());
					// Prepare attachment to be uploaded.
					...
					addDocumentToUpload(fileName1,fileInformationMap);
					addDocumentToUpload(fileName2,fileInformationMap);		
					
					// Send the request.
					currentRequest = appContext.accelaMobile4Citizen.request(servicePath, null, HTTPMethod.POST, requestParams, fileInformationMap, requestDelegate);
				}
				break;	
	...
	private void initContentView() {
		...
		this.mainLayout = (ViewGroup)this.findViewById(android.R.id.content).getRootView(); 
        this.btnCivicWebLogin = (Button) this.findViewById(R.id.btnCivicWebLogin);
        this.btnCivicEmbeddedWebLogin = (Button) this.findViewById(R.id.btnCivicEmbeddedWebLogin);
        this.btnGetRecords = (Button) this.findViewById(R.id.btnCivicGetRecords);        
        this.btnCreateRecord = (Button) this.findViewById(R.id.btnCivicCreateRecord);  
        this.btnCreateRecordWithAttachments = (Button) this.findViewById(R.id.btnCreateRecordWithAttachments);
        
        this.btnSearchRecord = (Button) this.findViewById(R.id.btnCivicSearchRecord); 
        ...
        this.btnCreateRecordWithAttachments.setOnClickListener(this);
       ...
       // Initialize app context.
		...
		appContext.accelaMobile4Citizen = new AccelaMobile(this, "634934818123833139", "15fbf8324ba84518b159a0c5b8c0f930", sessionDelegate,authServer,apiServer);         
		// Set the environment.
		appContext.accelaMobile4Citizen.setEnvironment(AccelaMobile.Environment.DEV);
		// Set the URL schema.
	...
	private boolean isSessionValid() {	
		Boolean isValid = (appContext.accelaMobile4Citizen != null) && (appContext.accelaMobile4Citizen.isSessionValid());
		if (!isValid) {
			Toast.makeText(this, this.getResources().getString(R.string.msg_not_logged_in), Toast.LENGTH_SHORT).show();
		}		
		return isValid;
	}
	...
	private AMSessionDelegate sessionDelegate = new AMSessionDelegate() {
		public void amDidLogin() {
			...
			// Show message.			
			Toast.makeText(CitizenTestActivity.this, CitizenTestActivity.this.getResources().getString(R.string.msg_logged_in), Toast.LENGTH_SHORT).show();
			AMLogger.logInfo("In CitizenTestActivity: Session Delegate.amDidLogin() invoked...");
		}		
		public void amDidLoginFailure(AMError error) {
			Toast.makeText(CitizenTestActivity.this, CitizenTestActivity.this.getResources().getString(R.string.msg_login_failed), Toast.LENGTH_SHORT).show();
			AMLogger.logInfo("In CitizenTestActivity: Session Delegate.amDidLoginFailure() invokded: %s", error.toString());
		}
		...
		public void amDidSessionInvalid(AMError error) {
			AMLogger.logInfo("In CitizenTestActivity: Session Delegate.amDidSessionInvalid() invoked: %s", error.toString());
		}
		public void amDidLogout() {		
			Toast.makeText(CitizenTestActivity.this, CitizenTestActivity.this.getResources().getString(R.string.msg_logged_out), Toast.LENGTH_SHORT).show();
			AMLogger.logInfo("In CitizenTestActivity: Session Delegate.amDidLogout() invoked...");						
		}
	};

	/**
	 * private variable, defined the request delegate to be used by normal request.
	*/
	private AMRequestDelegate requestDelegate = new AMRequestDelegate() {			
			...
			public void onStart() {		
				...
				currentRequest.setOwnerView(CitizenTestActivity.this.mainLayout, CitizenTestActivity.this.getResources().getString(R.string.msg_request_being_processed));
			}
			...
			public void onTimeout() {
				...
				// Show alert dialog.
				createAlertDialog(CitizenTestActivity.this.getResources().getString(R.string.error_request_timeout_title), CitizenTestActivity.this.getResources().getString(R.string.error_request_timeout_message));
			}
			...
			public void onSuccess(JSONObject responseJson) {
                ...
                createAlertDialog(CitizenTestActivity.this.getResources().getString(R.string.msg_request_completed_title), CitizenTestActivity.this.getResources().getString(R.string.msg_request_completed_message) + ": \n " + responseJson.toString());	
			}
			...
			public void onFailure(Throwable error) {	
				...
				createAlertDialog(CitizenTestActivity.this.getResources().getString(R.string.error_request_failed_title), amError.toString());
			}				
		};
				
			
		/**
		 * private method, used to populate the JSON data for the request which creates record.
		*/
	...
	//			recordJson.put("additionalTableInfo", new JSONArray().put(asitJson));			
		...
		private void addDocumentToUpload(String fileName, Map<String, String> fileInformationMap) {
		...
		private String getJson4CreateRecordWithAttachment() {
			return "{\"record\":{\"type\":{\"id\":\"Building-Mobile-Mobile-CXP\"},\"address\":{\"xCoordinate\":\"67.082199\",\"yCoordinate\":\"24.9056\",\"addressFormat\":\"812 9th St, Sacramento, CA, 95814, United States\",\"streetName\":\"812 9th St\",\"city\":\"Sacramento\",\"state\":\"CA\",\"postalCode\":\"95814\"},\"description\":\"Create from Catch Report\"}}";
		}
}